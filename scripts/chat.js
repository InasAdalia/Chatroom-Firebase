import { db, collection, serverTimestamp, addDoc, query, getDocs, where, orderBy, onSnapshot, updateDoc } from './firebase.js';
...
class Chatroom{
    ...
    updateName = async (oldName, newName) => {
        try {
            // Query documents where username matches oldName
            const userQuery = query(this.chatCol, where("username", "==", oldName));
            const querySnapshot = await getDocs(userQuery);
    
            // Loop through matching documents and update username field
